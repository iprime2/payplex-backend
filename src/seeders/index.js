...
const seed = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const files = (0, fs_1.readdirSync)(seedersPath).filter(file => file !== 'index.ts' && file.endsWith('.ts'));
    for (const file of files) {
        const filePath = path_1.default.join(seedersPath, file);
        const seederModule = yield Promise.resolve(`${filePath}`).then(s => __importStar(require(s)));
        if (typeof seederModule.default === 'function') {
            console.log(`🔹 Running: ${file}`);
            yield seederModule.default();
        }
        else {
